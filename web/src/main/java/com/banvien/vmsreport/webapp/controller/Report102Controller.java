...
import com.banvien.vmsreport.webapp.command.Report102Command;
 ...
 */
@Controller
public class Report102Controller extends ApplicationObjectSupport {
    private transient final Log log = LogFactory.getLog(this.getClass());
    private static final Integer TOTAL_COLUMN_EXPORT = 7;

    @Autowired
    private Report102ManagementLocalBean report102ManagementLocalBean;
    @Autowired
...
//        binder.registerCustomEditor(Date.class, new CustomDateEditor(Constants.DATE_FORMAT));
    }

    @RequestMapping(value = {"/admin/report/report102.*", "/normal/report/report102.*"})
    public ModelAndView report102(@ModelAttribute(value = Constants.FORM_MODEL_KEY) Report102Command command,
                                  HttpServletRequest request,
                                  BindingResult bindingResult,
                                  HttpServletResponse response) throws Exception {
        ModelAndView mav = new ModelAndView("/admin/report/report102");
    ...
    private void referenceData(Report102Command command, ModelAndView mav) {
    ...
    private void executeSearch(Report102Command command, HttpServletRequest request) throws ParseException {
        RequestUtil.initSearchBean(request, command);
        Map<String, Object> properties = buildProperties(command);
        Object[] results = report102ManagementLocalBean.search(properties, command.getSortExpression(), command.getSortDirection(), command.getFirstItem(), command.getMaxPageItems());
        command.setListResult((List<Report102DTO>)results[1]);
        command.setTotalItems(Integer.valueOf(results[0].toString()));
    }

    private Map<String, Object> buildProperties(Report102Command command) {
    ...
    private void export2Excel(Report102Command command, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try{
            SimpleDateFormat df = new SimpleDateFormat("dd-M-yyyy");
            Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
            String ngayXuatBaoCao = df.format(currentTimestamp);
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.report102ManagementLocalBean.search(properties, null, null, 0, Integer.MAX_VALUE);
            List<Report102DTO> dtoList = (List<Report102DTO>)resultObject[1];

            String outputFileName = "/files/temp/ThongKeCacGoiThauDenHanThucHien" + ngayXuatBaoCao + ".xls";
            String reportTemplate;
            reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/ThongKeCacGoiThauDenHanThucHien.xls");
    ...
    private void addHeader4Report(WritableSheet sheet, Report102Command command) throws WriteException{
        int columnIndex = 0;
...
//        ExcelUtil.mergeCells(sheet, 0, 4, TOTAL_COLUMN_EXPORT - 1, 4);
    ...
    private CellValue[] addCellValues(Report102DTO dto, Integer indexRow){
