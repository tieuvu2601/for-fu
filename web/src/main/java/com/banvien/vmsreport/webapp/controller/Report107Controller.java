...
import com.banvien.vmsreport.webapp.command.Report107Command;
 ...
 */
@Controller
public class Report107Controller extends ApplicationObjectSupport {
    private transient final Log log = LogFactory.getLog(this.getClass());
    private static final Integer TOTAL_COLUMN_EXPORT = 7;


    @Autowired
    private Report107ManagementLocalBean report107ManagementLocalBean;
    @Autowired
    ...
    @RequestMapping(value = {"/admin/report/report107.*", "/normal/report/report107.*"})
    public ModelAndView report107(@ModelAttribute(value = Constants.FORM_MODEL_KEY)Report107Command command,
                                  HttpServletRequest request,
                                  HttpServletResponse response) throws Exception {
        ModelAndView mav = new ModelAndView("/admin/report/report107");
    ...
    private void referenceData(Report107Command command, ModelAndView mav) {
    ...
    private void executeSearch(Report107Command command, HttpServletRequest request) throws ParseException {
        RequestUtil.initSearchBean(request, command);
        Map<String, Object> properties = buildProperties(command);
        Object[] resultsObject = report107ManagementLocalBean.search(properties, command.getSortExpression(), command.getSortDirection(), command.getFirstItem(), command.getMaxPageItems());
        command.setListResult((List<Report107DTO>) resultsObject[1]);
        command.setTotalItems(Integer.valueOf(resultsObject[0].toString()));
        //To change body of created methods use File | Settings | File Templates.
    }

    private Map<String, Object> buildProperties(Report107Command command) {
    ...
    private void export2Excel(Report107Command command, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try{
            SimpleDateFormat df = new SimpleDateFormat("dd-M-yyyy");
            Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
            String ngayXuatBaoCao = df.format(currentTimestamp);
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.report107ManagementLocalBean.search(properties, null, null, 0, Integer.MAX_VALUE);
            List<Report107DTO> dtoList = (List<Report107DTO>)resultObject[1];

            String outputFileName = "/files/temp/ThongKeCacGoiThauDenHanThucHienCuaThamDinh" + ngayXuatBaoCao + ".xls";
            String reportTemplate;
            reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/ThongKeCacGoiThauDenHanThucHienCuaThamDinh.xls");
    ...
    private void addHeader4Report(WritableSheet sheet, Report107Command command) throws WriteException{
        int columnIndex = 0;
...
//        ExcelUtil.mergeCells(sheet, 0, 4, TOTAL_COLUMN_EXPORT - 1, 4);
    ...
    private CellValue[] addCellValues(Report107DTO dto, Integer indexRow){
        ...
        if(dto.getThoiGianLap() != null){
            resValue[columnIndex++] = new CellValue(CellDataType.INT, dto.getThoiGianLap());
        }else{
            resValue[columnIndex++] = new CellValue();
        }
