...
import com.banvien.vmsreport.webapp.command.AbstractCommand;
...
public final class RequestUtil {
    ...
    public static void initSearchBean4report(HttpServletRequest request, AbstractCommand command) {
        if (command != null){
            ...
            String sPage = request.getParameter(new ParamEncoder(command.getTableId()).encodeParameterName(TableTagParameters.PARAMETER_PAGE));
            Integer page = 1;
            if (StringUtils.isNotBlank(sPage)) {
                try {
                    page = Integer.valueOf(sPage);
