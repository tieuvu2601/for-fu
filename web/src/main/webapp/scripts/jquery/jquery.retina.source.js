    ...
    $.fn.retina = function(retina_part) {
        ...
        if(window.devicePixelRatio >= 2) {
            this.each(function(index, element) {
                if(!$(element).attr('data-src')) return;

                var checkForRetina = new RegExp("(.+)("+settings['retina_part']+"\\.\\w{3,4})");
                if(checkForRetina.test($(element).attr('data-src'))) return;

                var new_image_src = $(element).attr('data-src').replace(/(.+)(\.\w{3,4})$/, "$1"+ settings['retina_part'] +"$2");
