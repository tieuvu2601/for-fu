...
<html lang="en">
<head>
    <title><fmt:message key="report101A.title_page"/></title>
    <meta charset="utf-8">
...
<c:url var="formlUrl" value="${prefix}/report/report101A.html"/>
<c:url var="tbcUrl" value="${prefix}/report/report101B.html"/>
<c:url var="tctyUrl" value="${prefix}/report/report101C.html"/>
<body>
    ...
    <form:form commandName="item" action="${formlUrl}" id="listForm" method="post" autocomplete="off" name="listForm" cssClass="form-horizontal">
        <section class="content-header">
            <ol class="breadcrumb">
                ...
                <li class="active"><fmt:message key="report101A.title_page"/></li>
            </ol>
                ...
                <div class="col-xs-12">
                    <div id="content" class="box box-solid box-primary">
                        <div class="box-header">
                            <h3 class="box-title"><fmt:message key="report101A.title_page"/></h3>
                            <div class="filter-group">
                                ...
                                <div class="filter-form large">
                                    <div class="filter-content">
                                        ...
                                        <div class="form-group">
                                            <label class="col-sm-4 control-label"><fmt:message key="label.branch"/></label>
                                            <div class="col-sm-7">
                                                <form:select id="branchMenu" path="pojo.branchCode" class="form-control" onchange="javascript: loadDistrictList();">
                                                    <option value=""><fmt:message key="label.all" /></option>
                                                    <c:forEach items="${branchList}" var="branch">
                                                        <option <c:if test="${item.pojo.branchCode eq branch.branchCode}">selected="true"</c:if> value=${branch.branchCode}>${branch.branchName}</option>
                                                    </c:forEach>
                                                </form:select>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-7">
                                                <form:select id="districtMenu" path="pojo.districtCode" class="form-control">
                                                    <option value=""><fmt:message key="label.all" /></option>
                                                    <c:forEach items="${districtList}" var="branch">
                                                        <option <c:if test="${item.pojo.districtCode eq branch.districtCode}">selected="true"</c:if> value=${branch.districtCode}>${branch.districtName}</option>
                                                    </c:forEach>
                                                </form:select>
                                            </div>
                                        </div>
                                ...
                                <div id="reportTableContainer" style="margin-top: 10px;">
                                    <div class="freezeTableHeader">
                                        <div class="table-left container-fixed-table-header">
                                            <table class="table table_vms table_report table-hover">
                                                <thead>
                                                <tr>
                                                    <th class="table_header_center nowrap rowspan2"><fmt:message key="label.cong_ty_2" /></th>
                                                    <th class="table_header_center nowrap rowspan2"><fmt:message key="label.quan" /></th>
                                                </tr>
                                                </thead>
                                            </table>
                                        </div>
                                                ...
                                                <table class="table table_vms table_report table-hover">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <c:if test="${valColspan > 0}">
                                                            <th colspan="${valColspan}" class="table_header_center nowrap"><fmt:message key="label.year" />&nbsp;${item.startDate.getYear() + 1900}</th>
                                                        </c:if>
                                                        ...
                                                        <c:forEach begin="${item.startDate.getMonth() + 1}" end="${item.endDate.getMonth() + 1}" var="month">
                                                            ...
                                                            <c:if test="${valColspan > 0}">
                                                                <th colspan="${valColspan}" class="table_header_center nowrap no_border_bottom"><fmt:message key="label.month" />&nbsp;${month}</th>
                                                            </c:if>
                                                        </c:forEach>
                                                    </tr>
                                                    <tr>
                                    ...
                                    <div class="container-scroll-body">
                                                ...
                                                <c:choose>
                                                    <c:when test="${not empty items.listResult && fn:length(items.listResult) gt 0}">
                                                        <c:forEach items="${items.listResult}" var="dto1" varStatus="count1">
                                                            <c:choose>
                                                                <c:when test="${count1.count == fn:length(items.listResult)}">
                                                                    <c:set var="trClass" value="total_row text-center" />
                                                                    <c:set var="colspan" value="${2}"/>
                                                                    <c:set var="stype" value="hidden"/>
                                                                </c:when>
                                                                <c:when test="${not empty dto1.branchName && count1.count != fn:length(items.listResult)}">
                                                                    <c:set var="trClass" value="branch" />
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <c:set var="trClass" value="" />
                                                                    <c:set var="colspan" value="${1}"/>
                                                                    <c:set var="stype" value=""/>
                                                                </c:otherwise>
                                                            </c:choose>
                                                            <tr class="${trClass}">
                                                                ...
                                                                <td class="nowrap ${stype}">
                                                                    <c:choose>
                                                                        <c:when test="${dto1.districtCode eq Constants.HCM1 || dto1.districtCode eq Constants.HCM2}">
                                                                            <fmt:message key="Thang_GoiCuoc.other"/>
                                                                        </c:when>
                                                                        <c:otherwise>
                                                                            ${dto1.districtName}
                                                                        </c:otherwise>
                                                    ...
                                                    <c:otherwise>
                                                        <tr>
                                                            <td colspan="9"><fmt:message key="label.msg_no_item_found"/></td>
                                        ...
                                        <div class="table-left container-scroll-content">
                                            <table class="table table_vms table_report table-hover" style="float: left;">
                                                <tbody>
                                                <c:choose>
                                                    <c:when test="${not empty items.listResult && fn:length(items.listResult) gt 0}">
                                                        <c:forEach items="${items.listResult}" var="dto1" varStatus="count1">
...
<script>
    $(document).ready(function(){
        ...
        $("#btnFilter").click(function(){
            ...
            if (lbc == 'TBC') {
                ...
                document.getElementById("listForm").action = "${tctyUrl}";
            }
            $("#listForm").submit();
        });
    ...
    function ajaxGetDistrictListByBranchId(){
        $('#districtMenu').find('option:not(:first-child)').remove();
//        selectFirstItemSelect2('#DistrictMenu');
        $.ajax({
            url: '<c:url value="/ajax/report/getDistrictListByBranchId.html" />',
            type: 'get',
            dataType: 'json',
            data: {branchCode: $('#branchMenu').val()},
            success: function(res){
                if(res.districtList != null){
                    $(res.districtList).each(function(index, el){
                        $('#districtMenu').append("<option value=" +el.districtCode+ ">" + $.trim(el.districtName) + "</option>");
                    });
                }
            }
        });
    ...
    function loadDistrictList(){
        var branchCode = $('#branchMenu').val();
        $('#districtMenu').find('option:not(:first-child)').remove();
        $('#districtMenu').find('option:first-child').attr('selected', true);
        $.ajax({
            url: '<c:url value="/ajax/report/getDistrictListByBranchId.html" />',
            type: 'GET',
            contentType: "application/json",
            data: {branchCode: branchCode != '' ? branchCode : 'ALL'},
//            dataType: 'json',
            success: function (res) {
                if (res.districtList != null) {
                    $(res.districtList).each(function (index, el) {
                        $('#districtMenu').append("<option value=" + el.districtCode + ">" + el.districtName + "</option>");
                    });
                }
            }
        });
