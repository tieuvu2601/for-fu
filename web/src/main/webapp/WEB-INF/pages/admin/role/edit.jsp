...
<c:url var="backUrl" value="${prefix}/role/list.html"/>
<c:url var="formUrl" value="${prefix}/role/edit.html"/>
...
<form:form commandName="item" id="formItem" action="${formUrl}" method="post" cssClass="form-horizontal" novalidate="novalidate">
    <section class="content-header">
        <ol class="breadcrumb">
            ...
            <li><a href="${backUrl}"><fmt:message key="role.manager"/></a></li>
            <li class="active">
                <c:choose>
                    <c:when test="${not empty item.pojo.roleId}">
                        <fmt:message key="role.manager.edit_role"/>
                    </c:when>
                    <c:otherwise>
                        <fmt:message key="role.manager.add_role"/>
                    </c:otherwise>
                </c:choose>
            </li>
        </ol>
            ...
            <div class="col-md-12">
                <div class="box box-solid box-primary">
                    <div class="box-header with-border">
                        <c:choose>
                            <c:when test="${not empty item.pojo.roleId}">
                                <h3 class="box-title"><fmt:message key="role.manager.edit_role"/></h3>
                            </c:when>
                            <c:otherwise>
                                <h3 class="box-title"><fmt:message key="role.manager.add_role"/></h3>
                            </c:otherwise>
                        </c:choose>
                    </div>
